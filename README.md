...
- **划词翻译**：选中文字点「翻译」（或 Alt+T）→ 译文浮层（Markdown 渲染），可一键把译文放成页面批注；译文自动收集到译文面板，也可粘贴文本翻译
...
- **组织**：分类（一篇一个）/ 标签（多对多）/ 合集（把一组论文打包，可留空）/ 阅读状态 / 收藏；管理弹窗支持改名、计数、一键筛选到该项；搜索（含全文）、筛选、排序、分页
- **合集页**：卡片总览（论文数 + 标题预览）→ 打开合集看论文、搜索加入、一键移出；支持改名与简介
...
E2E 覆盖（15 例）：首页加载、上传 PDF、详情渲染、笔记自动/手动保存、笔记浮窗拖动与固定、删除、
回收站恢复与彻底删除、笔记 Markdown 预览（含注入转义）、PDF 划段高亮、PDF 文字批注（新建/拖动/持久化/删除）、划词翻译（拦截接口，不消耗真实调用）、备份接口。
CI（`.github/workflows/ci.yml`）在每次 push 与 PR 上跑这两组测试。

## API 概览

```
GET    /api/papers                        列表（搜索/筛选/排序/分页；含 excludeCollections）
...
GET    /api/graph                         引用网络（后端保留，前端暂不展示）
```
